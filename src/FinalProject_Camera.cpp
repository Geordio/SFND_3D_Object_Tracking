...
int main(int argc, const char *argv[])
{
    ...
    int imgStepWidth = 1;  // 1== every image
    ...
    for (string det : detectors)
    {
        ...
        for (string desc : descriptors)
        {
            ...
            cout << "Combination Index: " << combinationIdx << endl;
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth)
            {
                ...
                // now have classified and boxed vehicles on camera image
                ...
                // Note that this makes filteringing in the LIDAR TTC method unnecessary....
                // working assumption is level road surface.
                ...
                if (dataBuffer.size() > 1) // wait until at least two images have been processed
                {
                    ...
                    for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
                    {
                        ...
                        if (currBB->lidarPoints.size() > 0 && prevBB->lidarPoints.size() > 0) // only compute TTC if we have Lidar points
                        {
                            ...
                            // logString =  ", ttclidar: " + to_string(ttcLidar) + ", ttccamera: " + to_string(ttcCamera);

                            logString = logCombination + "," + to_string(imgIndex) + ", ttclidar: " + to_string(ttcLidar) + ", ttccamera: " + to_string(ttcCamera);
                            logger1.WriteLine(logString);
                            bVis = true;
                            if (bVis)
                            {
                                ...
                                cv::resizeWindow(windowName, 1242, 375);
            ...
            if (bDebug)
            {
                for (auto datab : dataBuffer)
                    cout << "no of BBox: " << datab.boundingBoxes.size() << endl;
            }
            combinationIdx++;
        } // end of descriptors
